...
@IonicPage()
@Component({
  selector: 'page-edit-people-modal',
  templateUrl: 'edit-people-modal.html',
})

export class EditPeopleModalPage {
  // auto
...
showbanklistif:boolean=false;
  constructor(public navCtrl: NavController, public vendorservice: VendorServiceProvider, public loadingCtrl: LoadingController, private toastCtrl: ToastController, public navParams: NavParams, public countryprovider: CountryProvider, public alertCtrl: AlertController, private view: ViewController) {
  ...
  this.searchedtext="";
    ...
    this.vendorservice.getEmpDetails(navParams.get("userdata").company_id, navParams.get("userdata").empid,localStorage.getItem('token')).then((result) => {
      this.userRegData.role_name =result['employee']['company_role_id'];
      this.userRegData.first_name = result['employee']['user']['first_name'];
      this.userRegData.last_name = result['employee']['user']['last_name'];
      this.userRegData.email = result['employee']['user']['email'];
      this.userRegData.phone = result['employee']['user']['profile']['phone'];
      this.userRegData.date_of_birth = result['employee']['user']['profile']['birth_date'];
      this.userRegData.gender = result['employee']['user']['profile']['gender'];
      this.userRegData.user_house_nubmer = result['employee']['user']['profile']['house_nubmer'];
      this.userRegData.user_street = result['employee']['user']['profile']['street'];
      this.userRegData.user_zip = result['employee']['user']['profile']['zip_code'];
      // this.userRegData.user_bank = result['employee']['user']['profile']['user_bank'];
      this.userRegData.user_bank = this.searchedtext;
      this.userRegData.user_bank_accout = result['employee']['user']['profile']['bank_accout'];
      this.userRegData.user_tax_number = result['employee']['user']['profile']['tax_number'];
    //  this.userRegData.user_paypal = result['employee']['user']['profile']['paypal_details'];
    if(result['employee']['user']['profile']['paypal_details'] != null){
      this.userRegData.user_paypal=result['employee']['user']['profile']['paypal_details'];
    } else {
      this.userRegData.user_paypal="";
    }
   
      this.getStatebyid(result['employee']['user']['profile']['country_id'], result['employee']['user']['profile']['state_id']);
      this.getCities(result['employee']['user']['profile']['state_id'],result['employee']['user']['profile']['city'])
      this.countryNames = this.getCountries();
     this.countryid = result['employee']['user']['profile']['country_id'];
     console.log('country',this.countryname1);
        // this.loading.dismiss();
      }, (err) => {
      this.loading.dismiss();
      this.presentToast(err);
  ...
  */
  ...
  onSubmit = function (userregform) {
    ...
    if (this.addpeopleform.valid) {
      ...
      this.vendorservice.updateaddpeople(userregform,localStorage.getItem('company_id'),localStorage.getItem('token'),this.navParams.get("userdata").empid,this.searchedtext).then((result) => {
        this.loading.dismiss();
        if (result.status) {
